      ...
      <tr style="border-top:1px solid #e2e8f0">
        ...
        <td style="padding:.75rem;text-align:right">
          <div class="client-actions">
            <button class="client-favorite-button ${client.favorito ? 'active' : ''}" type="button" onclick="toggleClientFavorite('${client.id}')" title="${client.favorito ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}" aria-label="${client.favorito ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}"><i class="${client.favorito ? 'ph-fill' : 'ph'} ph-star"></i></button>
    ...
    return `
      <article class="client-card" style="animation-delay:${(i * 0.04).toFixed(2)}s;">
        <div class="client-header" onclick="openClientProfile('${client.id}')">
          ...
          <button class="client-favorite-button client-card-favorite ${client.favorito ? 'active' : ''}" type="button" onclick="event.stopPropagation(); toggleClientFavorite('${client.id}')" title="${client.favorito ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}" aria-label="${client.favorito ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}"><i class="${client.favorito ? 'ph-fill' : 'ph'} ph-star"></i></button>
        </div>
...
window.toggleClientFavorite = async id => {
  ...
  if (!client) return;
  try {
    await db.collection('clients').doc(id).set({ favorito: !client.favorito, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
  } catch (error) {
