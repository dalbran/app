  ...
  container.innerHTML = `
    ...
    <div class="produtos-view-header">
      ...
      <div class="produtos-action-btns">
        <button id="btn-mobile-import-menu" type="button" class="prod-icon-btn prod-icon-btn-outline" title="Importar / Exportar">
          <i class="ph ph-arrows-down-up"></i>
          <span class="prod-btn-label">Importar / Exportar</span>
        </button>
        <button id="btn-new-product" class="prod-icon-btn prod-icon-btn-primary" title="Novo Produto">
          <i class="ph ph-plus"></i>
          <span class="prod-btn-label">Novo Produto</span>
        </button>
      </div>
    </div>
      ...
      <div class="import-export-sheet">
        ...
        <div class="import-export-options">
          <button id="btn-import-json" class="import-export-btn">
            ...
            <span class="ie-label">Importar JSON</span>
          </button>
          <button id="btn-export-json" class="import-export-btn">
            <span class="ie-icon-wrap"><i class="ph ph-download-simple"></i></span>
            <span class="ie-label">Exportar JSON</span>
          </button>
        </div>
        <button id="btn-close-import-export" class="import-export-close-btn">
    ...
    const variationsList = (prod.variacoes || []).map((v, index) => {
      ...
      return `
        <div class="variation-block">
          <div class="variation-title"><i class="ph ph-drop"></i> <strong>${escapeProductHtml(v.volume || 'Padrão')}</strong> ${fragList ? `<small style="color:#64748b;">(${fragList})</small>` : ''}</div>
...
function addVariationRow(data = null) {
  ...
  row.innerHTML = `
    <div style="display:grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap:0.5rem; margin-bottom:0.5rem;">
      <div>
      ...
      <div>
        ...
        <input type="number" step="0.01" class="v-atacado form-control" value="${precoAtacado}" placeholder="0.00" required style="width:100%; padding:0.4rem;">
      </div>
      <div>
        <label style="font-size:0.75rem; font-weight:700;">Varejo (R$)</label>
        <input type="number" step="0.01" class="v-varejo form-control" value="${precoVarejo}" placeholder="0.00" required style="width:100%; padding:0.4rem;">
      </div>
      <div>
        <label style="font-size:0.75rem; font-weight:700;">Nota Fiscal (R$)</label>
        <input type="number" step="0.01" class="v-nota-fiscal form-control" value="${precoNotaFiscal}" placeholder="Usa varejo" style="width:100%; padding:0.4rem;">
      </div>
      <div>
        <label style="font-size:0.75rem; font-weight:700; color:#b45309;">Especial / DF (R$)</label>
        <input type="number" step="0.01" class="v-especial form-control" value="${precoEspecial}" placeholder="Usa atacado" style="width:100%; padding:0.4rem;">
      </div>
    </div>
    <div>
