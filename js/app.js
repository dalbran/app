...
function setupPersonalPreferences() {
  ...
  const apply = preferences => {
    const size = Number(preferences.fontSize) || 16;
    document.documentElement.style.fontSize = `${size}px`;
    document.documentElement.style.setProperty('--app-font-scale', (size / 16).toFixed(3));
    document.documentElement.style.setProperty('--ui-font-size', `${size}px`);
    document.body.classList.toggle('theme-dark', preferences.theme === 'dark');
  };
  const read = () => {
    try {
      return JSON.parse(localStorage.getItem(key())) || { fontSize: 16, theme: 'light' };
    } catch {
      return { fontSize: 16, theme: 'light' };
    }
  };
  const save = (preferences, notify = false) => {
    localStorage.setItem(key(), JSON.stringify(preferences));
    apply(preferences);
    if (notify) showToast(`Tamanho da fonte: ${preferences.fontSize}px`, 'info');
  };
  apply(read());
  ...
  if (btnDec) {
    btnDec.onclick = () => {
      ...
      const current = Number(preferences.fontSize) || 16;
      preferences.fontSize = Math.max(12, Math.round((current - 1.5) * 10) / 10);
  ...
  if (btnInc) {
    btnInc.onclick = () => {
      const preferences = read();
      const current = Number(preferences.fontSize) || 16;
      preferences.fontSize = Math.min(24, Math.round((current + 1.5) * 10) / 10);
      save(preferences, true);
    };
  }
