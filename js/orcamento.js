...
function getPdvQuickQuantityModal() {
  ...
  modal.innerHTML = `<div class="pdv-quantity-backdrop"></div><section class="pdv-quantity-sheet" role="dialog" aria-modal="true" aria-labelledby="pdv-quantity-title"><div class="sheet-handle"></div><button type="button" class="pdv-quantity-close" aria-label="Fechar"><i class="ph ph-x"></i></button><h3 id="pdv-quantity-title"></h3><p class="pdv-quantity-instruction">Escolha a litragem para continuar.</p><div class="pdv-volume-options"></div><div class="pdv-fragrance-options hidden"></div><div class="pdv-quantity-controls hidden"><label>Quantidade de unidades</label><div><button type="button" data-pdv-quantity="-1">−</button><input id="pdv-quick-quantity-input" type="number" min="1" value="1" inputmode="numeric"><button type="button" data-pdv-quantity="1">+</button></div><button type="button" class="pdv-quantity-confirm">Adicionar ao carrinho</button></div></section>`;
  document.body.appendChild(modal);
  const close = () => { modal.classList.add('hidden'); modal.classList.remove('open'); };
  ...
  modal.querySelector('.pdv-volume-options').onclick = event => {
    ...
    if (documentMode === 'orcamento' && selectedVar && selectedVar.fragrancias && selectedVar.fragrancias.length > 0) {
      ...
      modal.querySelector('#pdv-quick-quantity-input').focus();
    }
  };
...
function renderMobileOrcamentoView(viewId = 'view-orcamento', mode = 'orcamento') {
  ...
  // Atualiza theme-color da navbar do SO para combinar com o header do PDV
  const metaTheme = document.querySelector('meta[name="theme-color"]');
  if (metaTheme) {
    metaTheme.setAttribute('content', isPdv ? '#0f172a' : '#1e293b');
  }
  ...
  container.innerHTML = `
    ...
    ${isPdv ? `
      <div class="header-pdv mobile-only-header">
        <div class="brand-info">
          ...
          <div>
            <div class="brand-title">PDV Vendas Mobile</div>
            <div class="brand-subtitle">Dalbran Distribuidora</div>
          </div>
        </div>
    ...
    <div class="tab-toggle">
      ...
      <button class="tab-btn" id="tab-btn-salvos" onclick="switchDocumentTab('salvos')">
        <i class="ph ${isPdv ? 'ph-receipt' : 'ph-clock-counter-clockwise'}"></i> ${isPdv ? 'Histórico Vendas' : 'Orçamentos Salvos'} (<span id="tab-saved-count">${savedCount}</span>)
      </button>
    </div>
    ...
    <div id="tab-content-novo" class="tab-content active">
      ...
      <section class="section-card step-section step-section-finish" data-step="3" id="section-finish-card">
        ...
        <div class="form-row">
          ...
          <div class="form-group" style="flex:1;">
            ...
            <input type="number" step="0.50" id="orc-desconto" class="form-control" value="0.00" min="0">
          </div>
        </div>
      ...
      <div class="${isPdv ? 'summary-pdv' : 'summary-card'}">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:10px;">
          <button id="btn-whatsapp-orcamento" class="btn btn-outline" style="border-color:#10b981; color:#10b981; background:white;" type="button">
            <i class="ph-fill ph-whatsapp-logo"></i> ${isPdv ? 'Compartilhar' : 'WhatsApp'}
          </button>
          ...
          <div class="product-form">
            ...
            <div class="field field-qty">
              ...
              <div class="qty">
                ...
                <input type="number" id="orc-input-qtd" min="1" value="1">
      ...
      <aside class="panel step-section-finish">
        ...
        <div class="payment">
          ...
          <div class="field">
            ...
            <input type="number" id="orc-desconto" value="0.00" min="0">
          </div>
          <div class="field">
      ...
      return `
        <div class="sale-history-card" style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:14px 16px; margin-bottom:12px; display:flex; justify-content:space-between; align-items:center; box-shadow:0 2px 8px rgba(0,0,0,0.04);">
          ...
          <div style="text-align:right;">
            <div style="font-size:0.95rem; font-weight:800; color:#0f172a;">${total}</div>
            <button class="btn-reopen-sale" type="button" onclick="openSavedQuoteActions('${doc.id}')" style="background:#f1f5f9; border:1px solid #e2e8f0; padding:6px 12px; border-radius:6px; font-size:0.75rem; font-weight:700; margin-top:6px; cursor:pointer;">Ver Recibo</button>
          </div>
        </div>
      `;
    } else {
...
window.openSavedQuoteActions = function(id) {
  ...
  document.getElementById('btn-print-saved').onclick = () => {
    // If it's a venda and we are on mobile, we can optionally use openThermalReceiptModal
    // but printQuote works directly.
    printQuote('80mm');
  };
...
function generateReceiptCanvasImage() {
  ...
  link.download = `Recibo_${lastFinalizedSale.numero || 'Venda'}.png`;
...
function generateCartRows(forceDesktop = null) {
  ...
  if (desktop) {
    ...
    return cart.map((item, index) => `
      <tr><td><strong>${escapeProductHtml(item.nome)}</strong> (${escapeProductHtml(item.volume)})<br><small>Frag: ${escapeProductHtml(item.fragrancia || 'Padrão')}</small></td><td><div class="quantity-control quantity-control-small"><button type="button" onclick="changeCartQuantity(${index}, -1)">−</button><input type="number" min="1" value="${item.quantidade}" onchange="setCartQuantity(${index}, this.value)"><button type="button" onclick="changeCartQuantity(${index}, 1)">+</button></div></td><td>${formatCurrency(item.precoUnitario)}</td><td>${formatCurrency(item.subtotal)}</td><td><button type="button" class="desktop-remove-item" onclick="removeCartItem(${index})">✖</button></td></tr>`).join('');
  }
  if (cart.length === 0) {
...
function runNativePrint(printArea) {
  if (!printArea?.innerHTML.trim()) {
    showToast('Não foi possível gerar o comprovante para impressão.', 'error');
    return;
  }
  printArea.style.display = 'block';
  // Força o navegador a calcular o conteúdo antes de abrir a caixa de impressão.
